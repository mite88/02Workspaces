...
public class EditController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		} else if (ofileChack != null && ofileChack.equals("Ok")) {
			// 기존 파일 삭제
			FileUtil.deleteFile(req, "/Uploads", prevSfile);
			dto.setOfile("");
			dto.setSfile("");
		...
		*/
		...
		if (result == 1) {
			...
			req.getRequestDispatcher("../BoardSkin/view.do?idx=" + idx).forward(req, resp);
			// resp.sendRedirect("../BoardSkin/view.do?idx="+idx);
			// JSFunction.alertLocation(resp, "수정완료", "../BoardSkin/view.do?idx="+idx);
		} else {
			JSFunction.alertLocation(resp, "비번검증 다시진행해주세요", "../BoardSkin/view.do?idx=" + idx);
		}
	}
}
